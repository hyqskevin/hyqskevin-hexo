---
title: Hermes vs OpenClaw（中）：运行时行为
date: 2026-09-02 00:00:00
description: Hermes vs OpenClaw 中篇：配置热加载（cached-agent signature 自动 rebuild）、OpenClaw 单点 gateway 故障模式（Issue #13758 / #63643 内存泄漏）、同一条飞书消息在两边分别走什么路。
series:
...
- **API key / 工具 / skill 改完必须 reload 路径**
...
- 下一条消息发出去，`agent.log` 立刻出现：
  ...
  ```
- **没有重启 gateway，模型就已经切到 M3**

这就是官方那段"cached-agent signature + 自动 rebuild"行为的活样本。
...
### 5.1 OpenClaw 已知故障（官方 Issue Tracker）
...
**关键观察**：Hermes 在第 ③ 步发生**进程分裂**——从这一刻起 gateway 不再负责具体执行，只负责消息通道；OpenClaw 从头到尾在同一进程内跑完全部 7 步。

## 下期预告

（下）篇讲子系统细节——沙箱、浏览器、Memory、Skills 四块横向对比，再给一张选型决策树帮你判断"我该上哪个"。👉 [（下）：子系统与选型](/2026/09/02/hermes-vs-openclaw-gateway-3/)
